...
if __name__ == "__main__":
	...
	fig_c = tp.error_plot(x=x, y=y, y_lower=y_lower, y_upper=y_upper, line_label='HCP', color='rgb(20, 20, 200)')

	#simulated graphs
	#path_to_graphs = 'data/simu_graph/0/noise_800,outliers_16/graphs/'
	path_to_graphs = './data/OASIS_graphs_both_hemi/OASIS_left/'
	...
	y_lower = y - np.std(degree_histo, 0)
	# error plot from real data
	fig_c2 = tp.error_plot(x=x, y=y, y_lower=y_lower, y_upper=y_upper, line_label='OASIS', color='rgb(200, 20, 20)')
	fig_c.extend(fig_c2)
	fig = go.Figure(fig_c)
	...
	fig.update_layout(
		...
		title='HCP vs OASIS degree dist',
		hovermode="x",
		font=dict(
