...
def node_to_node_affinity(graph_1, graph_2, kernel_args):
    ...
    for node_1 in graph_1.nodes:
        for node_2 in graph_2.nodes:
            if (not graph_1.nodes[node_1]["is_dummy"]) and (not graph_2.nodes[node_2]["is_dummy"]):
                kernel_value = kernel(graph_1.nodes[node_1]["sphere_3dcoords"], graph_2.nodes[node_2]["sphere_3dcoords"], kernel_args)
                affinity_matrix[node_1, node_2] = kernel_value
            else:
                affinity_matrix[node_1, node_2] = 0
...
def load_generate_and_save_affinity_and_incidence_for_pair(path_to_folder, graph_nb_1, graph_nb_2, kernel_args):
    ...
    # Do everything only if the file does not already exists
    path_to_affinity = os.path.join(path_to_folder, "affinity", "affinity_"+str(graph_nb_1)+"_"+str(graph_nb_2)+".mat")
    if not os.path.exists(path_to_affinity):
...
def generate_and_save_all_affinity_and_incidence_in_path(path_to_folder, kernel_args, nb_workers=4):
    ...
    if not os.path.isdir(os.path.join(path_to_folder, "affinity")):
        os.mkdir(os.path.join(path_to_folder, "affinity"))
    ...
    for i_graph in range(nb_tot_graphs):
        for j_graph in range(i_graph+1, nb_tot_graphs):
            ...
            list_arguments.append((path_to_folder, i_graph, j_graph, kernel_args))
